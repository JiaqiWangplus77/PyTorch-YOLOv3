...
if __name__ == "__main__":
    ...
    with open(os.path.join(opt.output_image_folder,'A_note.txt'),'w') as f:
    ...
    for batch_i, (img_paths, input_imgs) in enumerate(dataloader):
        # Configure input
        #input_imgs = input_imgs.convert('RGB')
...
#        img = np.stack((img,)*3,axis=-1)
        ...
        path = os.path.join(opt.output_image_folder,f"{filename}.png")
